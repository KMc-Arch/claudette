...
| # | Spec said | Decided | Why |
|---|---|---|---|
| D16 | §9 planner out of scope; O2 sentinel deferred, "needs reconciling with backlog-pop as a second ingestion path" | **The planner's output IS the flag.** `/hb-send` produces the outbox item; its presence in `~outbox/hb/` is the raised flag. NO separate sentinel/dirty-bit — one ingestion path. (Discovery still gates on `roots.db`: a root not registered there is skipped — see §9 and round-2 #13; not a second *ingestion* path, but a real registration prerequisite.) | Deletes O2 by construction; folds the flag into the step that already runs (no parallel mechanism). |
...
| D20 | (§10.2 three expected termini) | **`blocked-on-decision` is a fourth EXPECTED terminus** (inflight→go; consumes the item, not retried; pushes its branch if it has commits but opens **no PR**; only on a clean exit). Plus the **checkpoint-commit protocol** (commit-before-attempt; reset on regress; commit history = decision ledger) and an outcome `## Decisions` block. | A clean hand-back on an above-envelope fork is a good night, not a corpse. Git is the undo and the audit trail. |
...
### mileqa round-1 fixes (2026-09-04)

| # | Fix | Why |
|---|---|---|
| D22 | **`send`/`approve` refuse when `HB_SANDBOX` is set** — the worker can no longer queue/approve its own work. | I7/§8.6: the LLM is never a state actor. Converts prose rule 6 to an env gate (deliberate unset is the BL-53 ceiling). |
| D23 | **`send` hard-backstops the solvency bar the writer can enforce**: `objective` + `acceptance` required; scope entries must be relative, `..`-free, and resolve inside root (an absolute/`..` entry passed `exists()` yet never matched → silent permanent withheld-publish). | Single-owner principle: the two fields that DEFINE solvency, and the scope gate, get deterministic teeth, not LLM-prose alone. |
| D24 | **PR opens only on a converged terminus** (branch still pushed for blocked/exhausted/cap). Plus **`~inbox/hb/outcomes.jsonl`** — one stable-schema record per item handled, cross-run (the eventual web view's source). | gh is the wrong surface for unfinished work; the ledger is the durable roster (user ruling). |
| D25 | **`loose`/`god` require a non-empty `write_scope`** (hard-fail, not warn); god's rendered rule now says "any decision NOT in the forbid list" (was a contradiction with the forbid block). | Bound god: an unbounded-decision worker must still have a stated publish boundary (user ruling). Forbid binds every level. |

### audit round-2 fixes (2026-09-04)

**Boundary model ruling (user):** boundaries are sacrosanct as written and confirmed at /hb-send triage. Two tiers — STRUCTURAL path (`write_scope` allow + `write_forbid` deny, machine-enforced, absolute) and DECISION prose (`autonomy`/`forbid`/`pre_auth`, behavioural). /hb-send *schematizes* prose path-intent into the structural fields; the runtime gate never parses prose. Breach ⇒ block the WHOLE push (never strip), computed over the union of the pushed range.

| # | Fix |
|---|---|
| D26 | `write_forbid` + `read_forbid` added; `scope_breach` = outside-write_scope ∪ under-write_forbid (deny wins); breach withholds the whole push and flags `boundary_violation`. |
| D27 | **#12** scope gate + `files_touched` now use the **union of every commit** in `base..branch` (was the endpoint diff — a reverted-in-range off-scope commit still pushes). |
| D28 | **#11** `pre_auth` reframed as decision-only (never a path grant); the "may touch Y outside write_scope" example removed. **#2** `render_prompt` escapes `{{` in every value (a forbid/scope entry holding `{{ITEM_BODY}}` no longer injects). |
| D29 | **#1** `/hb-send` scratch writes inside `^` (was `^/^/…`, blocked by containment from a child). **#8** `send`/`approve` default to `caller_root()` (the project you're in); `--project` is the override. **#3** `.claude/skills/hb-send/` shim added. |
| D30 | **#9** `send` runs `project_errors` (rejects a non-repo-top-level project the pop gate would reject). **#5-residual** `parse_item` enforces loose/god→write_scope at pop (hand-edited items). **#14** `qa` validated against the enum. **#15** `autonomy_of` consults `cfg`. **#6** `render_prompt` does `AUTONOMY_RULE[lvl]` (no silent bounded-fallback) + import assert that rules cover all levels + per-level render tests. **#13** `roots()` logs apex-only fallback; spec corrected ("roots.db is the discovery gate"). **#10** `~outbox/start.md` is now a message-bus intent statement; the schema lives once in §10.7 + the command. |

**Out-of-subject → backlog:** #4 (scrub pre-push hook lets worker file-content bytes past via `scrub:allow` pragma / >col-2000 / binary assets) — pre-existing scrub subsystem, made more pressing by unattended `god`. **Already fixed in e253c05:** #7 (HB_SANDBOX refusal; forgery vector closed).
