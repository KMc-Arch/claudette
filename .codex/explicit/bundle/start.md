...
1. Copy the full project tree into the output folder — excluding `.state/bundles/` (a bundle must never recursively copy prior bundles or its own in-progress output), `.state/traces/`, and `.claude/` (all per the Rules below), and the project's own root `CLAUDE.md` — top level only — e.g. `rsync -a --no-links --exclude=/CLAUDE.md --exclude=/.state/bundles/ --exclude=/.state/traces/ --exclude=/.claude/ <src>/ <dst>/`. The trailing `/` on the source matters (without it the anchored excludes miss), and `--no-links` leaves symlinks out: recreating them in the bundle would be creating symlinks, which is human-only; nested projects' `CLAUDE.md` files are copied as-is. Steps 3–5 create the root one. If the output folder already holds a `CLAUDE.md` (a reused folder, or a first attempt that went wrong), start a new bundle folder rather than replace it.
