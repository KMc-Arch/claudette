...
1. Copy the full project tree into the output folder — excluding `.state/bundles/` (a bundle must never recursively copy prior bundles or its own in-progress output), `.state/traces/`, and `.claude/` (all per the Rules below), and the project's own root `CLAUDE.md` (steps 3–5 create it).
