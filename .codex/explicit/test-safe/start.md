---
version: 7
short-desc: Read-only structural validation (78 checks, safe anytime)
reads:
  - "^/.codex/"
  - "^/.state/"
  - "^/.claude/"
...
**Migration gate (identity spine).** Read `PRAGMA user_version` and the `agent_registry` columns (`PRAGMA table_info(agent_registry)`). The db is MIGRATED when `user_version >= 1` AND `agent_registry` carries a `root_id` column; otherwise it is PRE-MIGRATION (the live db migrates on the next real `cboot --materialize-only`). When PRE-MIGRATION, the three spine-dependent sub-checks below (2, 4, 6) print `[SKIP]` with `roots.db not yet migrated to the identity spine — run cboot --materialize-only` and are excluded from the parent verdict (never `[FAIL]`); the remaining sub-checks (1, 3, 5, 7) run in both states.

CURRENT = the current claims and each claim's *current* location. When MIGRATED, a moved root's `agent_registry.rel_path` is frozen claim-time history, so source the live location from the spine:
```
SELECT ar.agent_name, ar.agent_file, ar.root_id,
       COALESCE(rr.rel_path, ar.rel_path) AS rel_path
FROM agent_registry ar
LEFT JOIN roots_register rr ON rr.root_id = ar.root_id AND rr.valid_to IS NULL
WHERE ar.valid_to IS NULL
```
When PRE-MIGRATION, `agent_registry.rel_path` is still the live location:
```
SELECT agent_name, agent_file, rel_path FROM agent_registry WHERE valid_to IS NULL
```
**CURRENT is the sole authority on which files cboot owns** — ownership is this lookup, never an inference from a file's contents. A file's marker is checked below only to detect tampering with a file the registry already claims; it never adds a file to CURRENT and never removes one. Read a file in `^/.claude/agents/` with `errors="replace"` and treat an unreadable one as "no marker" — a file that cannot be decoded is a reporting matter, never a verdict on ownership.

Seven sub-checks:
1. **Apex is never addressable.** No CURRENT row has `rel_path = '.'`. `[FAIL]` — the apex IS the session, not a subagent.
2. **Every claim has a decision.** *(Spine-dependent — `[SKIP]` pre-migration.)* `agent_optin` exists, and every CURRENT claim has an `enabled = 1` decision. The re-keyed `agent_optin` is keyed on `root_id` (its `rel_path` is demoted to unmaintained history), so join it on the claim's `root_id` — `SELECT enabled FROM agent_optin WHERE root_id = ?` — not `rel_path`. `[FAIL]` per claim with no decision or a decision of 0 — a claim the apex cannot justify means the two tables have drifted.
...
4. **Claimed files still carry their marker.** *(Spine-dependent — `[SKIP]` pre-migration.)* For each CURRENT row whose file exists, its first non-blank line after the frontmatter block must fullmatch `<!-- cboot:agent root=(?P<root>"(?:[^"\\]|\\.)*") generated="[^"]*" -->` with `json.loads(root)` equal to the row's CURRENT `rel_path` (the spine's live location — a moved root's marker is regenerated to its new rel_path each boot, so comparing against the frozen `agent_registry.rel_path` would false-fail). Mechanically: strip a leading UTF-8 BOM; if the text up to the first `\n`, stripped, is exactly `---`, find the closing fence with `re.search(r"^---[ \t]*$", rest, re.M)` (no closing fence = the whole text is the body) and start the body after it; take the body's first line that is non-blank when stripped; `re.fullmatch` the marker on it. `[WARN]` per file that fails — cboot's own response is to warn and leave the file untouched, never to overwrite, so a diverged file is a report, not a defect in the tool.
...
Parent verdict is the worst across sub-checks; print offenders only. A `[SKIP]` sub-check (a spine-dependent check on a pre-migration db) does not contribute to the parent verdict — if every non-skipped sub-check passes, the parent is `[PASS]`. Stated caveat, inherited from T62: with `immutable=1` the verdict is undefined if anything writes roots.db during the open, and uncheckpointed `-wal` frames from a crashed writer are invisible.

**T65** — `roots_register` identity spine (if migrated)
Condition: SKIP if `.state/roots.db` is absent, OR if it is present but not yet migrated to the identity spine — `PRAGMA user_version < 1` (the durable `roots_register` spine is introduced by the `user_version` 0→1 ladder on the next real `cboot --materialize-only`; until then there is nothing to assert). Print `[SKIP]` with `roots.db not yet migrated to the identity spine — run cboot --materialize-only`. Otherwise open the db STRICTLY READ-ONLY exactly as T62/T64 mandate: `sqlite3.connect("file:<abs-path>?immutable=1&mode=ro", uri=True)` — `immutable=1` is the operative flag, `mode=ro` the belt — NOT through the sqlite factory (its `PRAGMA journal_mode=WAL` persists a journal-mode flip into the file). Short-lived: open, read, close. An error opening read-only is a `[FAIL]` reporting the exception text.

Five sub-checks:
1. **Spine table shape.** `roots_register` contains AT LEAST the columns `id, root_id, rel_path, is_apex, change_reason, valid_from, valid_to` (superset semantics; `PRAGMA table_info(roots_register)`). `[FAIL]` naming any missing column.
2. **Current-row partial-unique indexes.** Among the indexes on `roots_register` (`SELECT name, sql FROM sqlite_master WHERE type='index' AND tbl_name='roots_register' AND sql IS NOT NULL`), there is a UNIQUE partial index whose definition covers `root_id` and carries `WHERE valid_to IS NULL`, AND another whose definition covers `rel_path` (with `COLLATE NOCASE`) and the same `WHERE valid_to IS NULL` predicate. `[FAIL]` if either is absent — without them two current rows could share a `root_id` or a live `rel_path`.
3. **Exactly one current apex.** `SELECT rel_path, root_id FROM roots_register WHERE valid_to IS NULL AND is_apex = 1` returns exactly one row and its `rel_path` is `.`. `[FAIL]` otherwise — the apex is a root and must hold exactly one live identity.
4. **The apex holds an identity.** That one current apex row's `root_id` is non-null. `[FAIL]` otherwise.
5. **Every current claim is linked to the spine.** Every CURRENT `agent_registry` claim (`SELECT root_id FROM agent_registry WHERE valid_to IS NULL`) has a non-null `root_id` that matches a CURRENT `roots_register` row (a `roots_register` row with that `root_id` and `valid_to IS NULL`). `[FAIL]` per claim whose `root_id` is null or resolves to no current spine row — a claim not anchored in the identity spine is orphaned.

Parent verdict is the worst across sub-checks; print offenders only. Same `immutable=1` caveat as T62/T64.
