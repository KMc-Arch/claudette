...
- `test_claude_md_guard.sh` — `claude-md-immutability-guard.sh`: EVERY CLAUDE.md
  (case-insensitive name, a hardlink in the same directory, any depth — a parent
  ...
  CR/BOM/non-LF files and results are refused; creating a new CLAUDE.md is
  allowed; every unvettable input fails **closed**. Payloads are raw UTF-8, as
  Claude Code sends them. Needs a case-sensitive `TMPDIR` for the lowercase
  `claude.md` case to mean what it says.
  Run: `bash test_claude_md_guard.sh` — exit 0 = all pass.

- `mutate_claude_md_guard.sh` — the mutation proof for the suite above: 41
  mutants, each reverting one hardening, each required to turn the suite red;
  the four controls it deliberately does not mutate (no verdict changes on this
  platform) are named, with the reason, in its header.
  Run: `bash mutate_claude_md_guard.sh` — exit 0 = every mutant caught. ~5 minutes.
