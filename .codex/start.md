...
| `.claude/agents/` | (a) project agents generated by `cboot.py::generate_agents` from the opt-in decisions held in `.state/roots.db`; (b) reserved for modules with `isolation: subagent` (shim generation not yet implemented) |
...
A file in `.claude/agents/` belongs to cboot **if and only if the durable `agent_registry` in `.state/roots.db` currently claims it** — ownership is a path lookup, never a guess from the file's contents. The one implementation lives in `.codex/reactive/agent-ownership` and is shared with `purge`; neither may carry its own copy. The `<!-- cboot:agent root="…" generated="…" -->` marker on a generated file is advisory: a "do not hand-edit" banner and a tamper check, conferring nothing. A hand-authored file is never overwritten or deleted, whatever it contains, and reserves its `@name`.

**Nothing about addressability lives in the child.** A project is addressable because the apex's `roots.db` says so; its own `CLAUDE.md` is read for a default description and never written. cboot asks about a project it has not seen before — once, at a terminal — and keeps the answer forever.
