...
| `claude-md-immutability-guard.sh` | Changing any CLAUDE.md body or protected frontmatter key (`name:`/`orchestrator:` stay agent-editable; creating one is blocked too) | Write, Edit |
...
| `.claude/agents/` | Project agents, generated by `cboot.py` from the opt-in decisions in `.state/roots.db`. Module shims for `isolation: subagent` remain planned. |

The `.claude/` directory is regenerated each boot. Never edit it directly -- changes will be overwritten.

One exception: a file in `.claude/agents/` that cboot does not claim is yours. Ownership is a lookup against `.state/roots.db`, never a guess from the file's contents, so a hand-authored agent is never overwritten or deleted -- and a generated one you edit is left alone from then on, with a warning, rather than being silently rewritten.
